...
Matrix Conv::multiply(const Matrix& data_col, const Matrix& weight) {
  Matrix temp = weight;
  
  //use this when convert 6 to 16 maps in convo 2-nd
  if(type == 2) {
    int hw_kernel = height_kernel * width_kernel;
    
    // channel_out = 16
    // 6 first kernel do not need to multiply with some maps. So we will set to 0.
    
    int idx = 3;
    for(int c = 0; c < 6; c++) { // first 6 kernel
        int st = idx * hw_kernel;
        for(int cnt = 0; cnt < hw_kernel * 3; cnt++) {
            int r = (st + cnt) % (hw_kernel * channel_in);
            temp(r, c) = 0;
        }
        idx++;
    }
    ...
    idx = 4;
    for(int c = 6; c < 12; c++) { // next 6 kernel
        int st = idx * hw_kernel;
        for(int cnt = 0; cnt < hw_kernel * 2; cnt++) {
            int r = (st + cnt) % (hw_kernel * channel_in);
            temp(r, c) = 0;
        }
        idx++;
    ...
    idx = 2;
    for(int c = 12; c < 15; c++) { // next 3 kernel
        int st1 = idx * hw_kernel;
        int st2 = ((idx + 3) % channel_in) * hw_kernel;
        for(int r = st1; r < st1 + hw_kernel; r++) temp(r, c) = 0;
        for(int r = st2; r < st2 + hw_kernel; r++) temp(r, c) = 0;
        idx++;
    }

    //the last kernel, we keep it because last map connect to all 6 maps
    for(int c = 15; c < channel_out; c++);

  }

  return data_col * temp;
}
...
void Conv::forward(const Matrix& bottom) {
  ...
  for (int i = 0; i < n_sample; i ++) {
    ...
    // conv by product
    Matrix result = multiply(data_col, weight);  // result: (hw_out, channel_out)
